...
import ReactStars from "react-stars"; // Assuming you're using react-stars for the rating
import { IoHeartOutline } from "react-icons/io5";
import { TbTruckDelivery } from "react-icons/tb";
import { HiArrowSmDown } from "react-icons/hi"; // Corrected import for valid arrow icon
...
export default function ProductDetail({darkMode}) {
  ...
  useEffect(() => {
    if (product) {
      setQuantity(product?.minimumOrderQuantity || 1); // Default quantity
      setSelectedImage(product?.images?.[0] || ''); // Set first image by default
    }
  }, [data, product]);
      ...
      <div className=" mx-4 md:mx-16 xl:mx-[92px] ">
        <div className="product md:h-[calc(100vh-133px)] lg:mb-0 w-[calc(100%)] flex flex-col md:flex-row items-center gap-6 lg:gap-0 justify-between">
          ...
          <div className="product-images flex gap-3 sm:gap-6 md:gap-3 lg:gap-6 sm:items-center justify-center w-[100%] md:w-[50%] lg:w-[60%] py-6 sm:py-8 md:py-0 h-[calc(100vh-133px)] md:h-[300px] lg:h-[400px] xl:h-[450px] ">
                ...
                <div className="w-full flex flex-col gap-3">
                  {product?.images?.map((image, i) => (
                    <img
                      ...
                      src={image}
                      alt=""
                      className={`${darkMode ? "bg-gray-300" : "bg-secondary"} rounded h-[85px] sm:h-[105px] md:h-[92px] lg:h-[125px] xl:h-[142px] object-contain cursor-pointer`}
            ...
            <div
              className={`${
                darkMode ? "bg-gray-300" : "bg-secondary"
              } w-[75%] sm:w-[80%] md:w-[400px] rounded flex items-center justify-center h-[280px] sm:h-full`}
            >
              <img
          ...
          <div className="product-detail w-[100%] md:w-[50%] lg:w-[40%] h-[480px] flex flex-col gap-1 md:gap-0 lg:gap-[2px] xl:gap-1 md:py-[85px] lg:py-9 xl:py-3">
            <h2 className={`sm:text-lg md:text-base lg:text-lg font-semibold ${darkMode ? "text-white" : ""}`}>
              {product?.title}
            </h2>
            ...
            <p
              className={`${
                darkMode ? "text-white" : ""
              } text-xs sm:text-sm md:text-xs lg:text-sm mt-1 sm:mt-2 md:mt-1 lg:mt-3 w-full md:max-w-[400px] border-b border-y-gray-400 pb-2 lg:pb-4`}
            >
              {product?.description}
            </p>
            <div className="flex gap-3 items-center mt-4 lg:mt-5">
              <div className="w-32 sm:w-40 md:w-32 lg:w-40 h-10 sm:h-11 md:h-10 lg:h-11 border border-gray-400 rounded flex ">
                <button
                  ...
                  onClick={() => {
                    if (quantity < 2 || quantity <= product?.minimumOrderQuantity) return;
            ...
            >
              ...
              <div className="flex gap-4 xl:gap-4 items-center p-2 md:p-1 lg:p-3">
                <HiArrowSmDown className="text-2xl lg:text-3xl" /> {/* Updated icon */}
                <div className="flex flex-col gap-1 xl:gap-2">
